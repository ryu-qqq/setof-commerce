# ========================================
# ECS Service: legacy-batch (Stage)
# ========================================
# Spring Batch Jobs for Legacy System - Stage Environment
# - Sellic 외부몰 주문 동기화
...
resource "aws_kms_key" "logs" {
  description             = "KMS key for SetOf Commerce legacy-batch Stage CloudWatch logs encryption"
...
module "legacy_batch_logs" {
  ...
  retention_in_days = 14 # Stage는 14일 보관
...
module "ecs_security_group" {
  ...
  description = "Security group for legacy-batch ECS tasks Stage (internal only)"
  vpc_id      = local.vpc_id
...
# ========================================
# EventBridge Schedules for Batch Jobs (Stage)
# ========================================
# Stage 환경에서는 스케줄 간격을 조정하여 비용 절감
...
resource "aws_scheduler_schedule" "shipment_tracking" {
  ...
  schedule_expression = "rate(1 hour)" # Stage: 1시간 (Prod: 30분)
...
resource "aws_scheduler_schedule" "sellic_sync" {
  ...
  # Stage: 하루 3회 (09, 12, 18시)
  schedule_expression          = "cron(0 9,12,18 * * ? *)"
...
# ========================================
# Schedule: 완료 주문 상태 업데이트 (매 30분 - Stage 절감)
...
resource "aws_scheduler_schedule" "order_completed" {
  ...
  schedule_expression = "rate(30 minutes)" # Stage: 30분 (Prod: 15분)
...
# ========================================
# Schedule: 정산 주문 상태 업데이트 (매 30분 - Stage 절감)
...
output "task_definition_arn" {
  description = "ECS Task Definition ARN for legacy-batch Stage"
  value       = aws_ecs_task_definition.legacy_batch.arn
}

output "task_definition_family" {
  description = "ECS Task Definition Family for legacy-batch Stage"
  value       = aws_ecs_task_definition.legacy_batch.family
}

output "security_group_id" {
  description = "Security Group ID for legacy-batch Stage"
  value       = module.ecs_security_group.security_group_id
}

output "eventbridge_role_arn" {
  description = "EventBridge IAM Role ARN for batch scheduling"
