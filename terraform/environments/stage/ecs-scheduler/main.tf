...
# Add inbound rule to staging RDS security group for scheduler ECS access
resource "aws_security_group_rule" "scheduler_to_rds" {
  ...
  description              = "Allow scheduler-stage ECS to access staging RDS"
}
...
resource "aws_kms_key" "logs" {
  description             = "KMS key for SetOf Commerce scheduler-stage CloudWatch logs encryption"
...
module "ecs_security_group" {
  ...
  description = "Security group for scheduler ECS tasks (Stage) - Internal only"
  vpc_id      = local.vpc_id

  type = "custom"

  # Internal communication only - no external ingress needed
...
module "ecs_service" {
  ...
  # load_balancer_config is not set
  ...
  # No Service Discovery needed for scheduler
