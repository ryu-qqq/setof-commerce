...
# ========================================
# Features:
# - Legacy DB -> New DB data migration
...
resource "aws_kms_key" "logs" {
  description             = "KMS key for SetOf Commerce migration CloudWatch logs encryption"
...
module "migration_task_execution_role" {
  ...
  secrets_manager_secret_arns = [
    ...
    data.aws_secretsmanager_secret.legacy_rds.arn
  ]
...
module "ecs_service" {
  ...
  container_secrets = [
    ...
    { name = "LEGACY_DB_PASSWORD", valueFrom = "${data.aws_secretsmanager_secret.legacy_rds.arn}:password::" }
  ]
