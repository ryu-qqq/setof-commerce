package com.ryuqq.setof.storage.legacy.user.condition;

import static com.ryuqq.setof.storage.legacy.user.entity.QLegacyUserEntity.legacyUserEntity;

import com.querydsl.core.types.dsl.BooleanExpression;
import org.springframework.stereotype.Component;

/**
 * LegacyMemberConditionBuilder - 레거시 회원 QueryDSL 조건 빌더.
 *
 * <p>PER-CND-001: ConditionBuilder는 @Component로 등록.
 *
 * <p>PER-CND-002: 각 조건은 BooleanExpression 반환.
 *
 * <p>PER-CND-003: null 입력 시 null 반환 (동적 쿼리 지원).
 *
 * @author ryu-qqq
 * @since 1.2.0
 */
@Component
public class LegacyMemberConditionBuilder {

    /**
     * 사용자 ID 일치 조건.
     *
     * @param userId 사용자 ID
     * @return BooleanExpression
     */
    public BooleanExpression userIdEq(Long userId) {
        return userId != null ? legacyUserEntity.id.eq(userId) : null;
    }

    /**
     * 전화번호 일치 조건.
     *
     * @param phoneNumber 전화번호
     * @return BooleanExpression
     */
    public BooleanExpression phoneNumberEq(String phoneNumber) {
        return phoneNumber != null ? legacyUserEntity.phoneNumber.eq(phoneNumber) : null;
    }
}
