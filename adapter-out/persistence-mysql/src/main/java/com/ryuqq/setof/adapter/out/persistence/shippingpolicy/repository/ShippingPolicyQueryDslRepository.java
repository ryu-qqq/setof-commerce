...
/**
 * ShippingPolicyQueryDslRepository - ShippingPolicy QueryDSL Repository
 *
...
public class ShippingPolicyQueryDslRepository {
    ...
    private static final QShippingPolicyJpaEntity qPolicy =
            QShippingPolicyJpaEntity.shippingPolicyJpaEntity;

    public ShippingPolicyQueryDslRepository(JPAQueryFactory queryFactory) {
    ...
    /** ID로 ShippingPolicy 단건 조회 (삭제 포함) */
    public Optional<ShippingPolicyJpaEntity> findByIdIncludeDeleted(Long id) {
        return Optional.ofNullable(
                queryFactory.selectFrom(qPolicy).where(qPolicy.id.eq(id)).fetchOne());
    }

    /** 셀러 ID로 ShippingPolicy 목록 조회 */
    public List<ShippingPolicyJpaEntity> findBySellerId(Long sellerId, boolean includeDeleted) {
