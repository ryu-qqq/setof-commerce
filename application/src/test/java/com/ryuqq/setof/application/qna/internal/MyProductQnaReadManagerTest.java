...
@DisplayName("MyProductQnaReadManager 단위 테스트")
class MyProductQnaReadManagerTest {

    @InjectMocks private MyProductQnaReadManager sut;

    @Mock private QnaMyProductQueryPort queryPort;
    ...
    @Nested
    @DisplayName("fetchMyQnas() - 내 상품 Q&A 목록 조회")
    class FetchMyQnasTest {
        ...
        void fetchMyQnas_ValidCriteria_ReturnsMyQnaResultList() {
            // given
            List<MyQnaResult> expected = QnaQueryFixtures.myProductQnaResultList();
            given(queryPort.fetchMyProductQnas(criteria)).willReturn(expected);
            ...
            then(queryPort).should().fetchMyProductQnas(criteria);
        }
        ...
        void fetchMyQnas_NoQnas_ReturnsEmptyList() {
            // given
            given(queryPort.fetchMyProductQnas(criteria)).willReturn(List.of());
