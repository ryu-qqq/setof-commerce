...
/**
 * FetchProductGroupDetailService - 상품그룹 단건 상세 조회 Service.
 *
 ...
 */
@Service
public class FetchProductGroupDetailService implements FetchProductGroupDetailUseCase {

    private final ProductGroupReadFacade readFacade;

    public FetchProductGroupDetailService(ProductGroupReadFacade readFacade) {
        this.readFacade = readFacade;
    }

    @Override
    public ProductGroupDetailResult execute(Long productGroupId) {
        ProductGroupDetailCompositeResult composite = readFacade.getDetailBundle(productGroupId);
