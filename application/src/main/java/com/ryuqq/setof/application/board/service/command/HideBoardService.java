...
public class HideBoardService implements HideBoardUseCase {
    ...
    @Override
    public void execute(HideBoardCommand command) {
        BoardId boardId = BoardId.of(command.boardId());
        Board existing = boardReadManager.findById(boardId);
        Board hidden = existing.hide(command.updatedBy(), clockHolder.getClock().instant());
        boardPersistenceManager.persist(hidden);
    }
}
