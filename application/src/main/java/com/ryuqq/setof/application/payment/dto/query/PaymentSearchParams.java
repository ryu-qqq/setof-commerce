 ...
 * 결제 목록 조회 파라미터 DTO.
 *
 * <p>커서 기반 페이징 + 날짜 범위 + 주문 상태 필터를 포함합니다.
 *
 * @param userId 사용자 ID (필수)
 * @param startDate 검색 시작일
 * @param endDate 검색 종료일
 * @param orderStatuses 주문 상태 필터 목록
 * @param lastPaymentId 커서 페이징용 마지막 결제 ID (null이면 첫 페이지)
...
public record PaymentSearchParams(
    ...
    public static PaymentSearchParams of(
            long userId,
            LocalDate startDate,
            LocalDate endDate,
            List<String> orderStatuses,
            Long lastPaymentId,
            Integer size) {
        return new PaymentSearchParams(
                userId, startDate, endDate, orderStatuses, lastPaymentId, size);
    }
