 ...
 */
@Service
public class UpdateProductDisplayService implements UpdateProductDisplayUseCase {
    ...
    public UpdateProductDisplayService(
    ...
    @Override
    public void execute(UpdateProductDisplayCommand command) {
        ...
        Product updated = command.displayYn() ? product.show(now) : product.hide(now);
    ...
    private void validateOwnership(ProductGroup productGroup, Long sellerId) {
        if (!productGroup.getSellerIdValue().equals(sellerId)) {
            throw new IllegalArgumentException("해당 상품의 전시 상태 변경 권한이 없습니다");
        }
    }
}
