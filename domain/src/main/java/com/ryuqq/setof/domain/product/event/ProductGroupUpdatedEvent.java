 ...
 * 상품그룹 수정 이벤트
 *
 * <p>상품그룹이 수정되었을 때 발행되는 도메인 이벤트입니다.
 *
 * <p>Domain Event 규칙:
 *
 * <ul>
 ...
 */
public record ProductGroupUpdatedEvent(
     ...
     *
     * @param productGroup 수정된 상품그룹
     * @param occurredAt 이벤트 발생 시각
     * @return ProductGroupUpdatedEvent 인스턴스
     */
    public static ProductGroupUpdatedEvent from(ProductGroup productGroup, Instant occurredAt) {
        return new ProductGroupUpdatedEvent(
                productGroup.getId(),
                productGroup.getCategoryId(),
                productGroup.getBrandId(),
                productGroup.getName(),
                productGroup.getPrice(),
                productGroup.getStatus(),
                occurredAt);
    }
}
