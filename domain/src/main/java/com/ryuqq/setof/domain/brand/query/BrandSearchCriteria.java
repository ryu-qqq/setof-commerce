 ...
 * @param brandName 브랜드명 검색 (null이면 전체)
 * @param queryContext 정렬 및 페이징 정보
 * @author ryu-qqq
 * @since 1.0.0
 */
public record BrandSearchCriteria(
        Boolean displayed, String brandName, QueryContext<BrandSortKey> queryContext) {
    ...
    public static BrandSearchCriteria of(
            Boolean displayed, String brandName, QueryContext<BrandSortKey> queryContext) {
        return new BrandSearchCriteria(displayed, brandName, queryContext);
    }
    ...
    public static BrandSearchCriteria defaultOf() {
        return new BrandSearchCriteria(
                null, null, QueryContext.defaultOf(BrandSortKey.defaultKey()));
    }
    ...
    public static BrandSearchCriteria displayedOnly() {
        return new BrandSearchCriteria(
                true, null, QueryContext.defaultOf(BrandSortKey.defaultKey()));
    }

    /** 브랜드명 검색 조건이 있는지 확인 */
    public boolean hasBrandNameFilter() {
