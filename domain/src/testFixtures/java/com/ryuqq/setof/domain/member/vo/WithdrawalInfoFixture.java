 ...
 * WithdrawalInfo custom = WithdrawalInfoFixture.create(WithdrawalReason.OTHER, Instant.now());
 * }</pre>
 */
public final class WithdrawalInfoFixture {
    ...
    private static final Instant DEFAULT_WITHDRAWN_AT = Instant.parse("2025-01-01T00:00:00Z");
     ...
     *
     * @param reason 탈퇴 사유
     * @param withdrawnAt 탈퇴 시간 (UTC 기준)
     * @return WithdrawalInfo 인스턴스
     */
    public static WithdrawalInfo create(WithdrawalReason reason, Instant withdrawnAt) {
    ...
    public static WithdrawalInfo createNow(WithdrawalReason reason) {
        return WithdrawalInfo.of(reason, Instant.now());
    }
