...
> **목적**: Lock을 걸고 조회하는 CQRS Query Adapter 구현 가이드
...
Lock을 걸고 조회하는 Query(읽기) 담당으로, LockRepository에 위임하고 Mapper를 통해 Domain으로 변환하여 반환합니다.

### 책임
- ✅ Lock 조회만 담당 (비관락, 낙관락, For Update)
- ✅ LockRepository 호출
...
### 원칙 1: Lock 조회만 담당
- **Lock Query**: 비관락, 낙관락, For Update 조회만
- **Command**: 저장/수정/삭제는 CommandAdapter로
- **일반 Query**: Lock 없는 조회는 QueryAdapter로

### 원칙 2: LockRepository 위임 + Mapper 변환
```java
// ✅ LockRepository 호출 → Mapper 변환 → Domain 반환
public Optional<Bc> findByIdWithPessimisticLock(BcId id) {
    return lockRepository.findByIdWithPessimisticLock(id.getValue())
        .map(bcJpaEntityMapper::toDomain);
}

// ❌ 비즈니스 로직 금지
public Optional<Bc> findByIdWithPessimisticLock(BcId id) {
    Optional<Bc> bc = lockRepository.findByIdWithPessimisticLock(id.getValue())
        .map(bcJpaEntityMapper::toDomain);
...
// ❌ 원시 타입 사용 금지
public Optional<Bc> findByIdWithPessimisticLock(Long id) {  // 금지!
}
```

### 원칙 4: 6개 메서드 고정
- **비관락**: findByIdWithPessimisticLock, findByCriteriaWithPessimisticLock
- **낙관락**: findByIdWithOptimisticLock, findByCriteriaWithOptimisticLock
- **For Update**: findByIdForUpdate, findByCriteriaForUpdate

### 원칙 5: Lock 예외 명시 (JavaDoc)
```java
...
public interface {Bc}LockQueryPort {
    ...
    Optional<Bc> findByIdWithPessimisticLock({Bc}Id id);
    List<Bc> findByCriteriaWithPessimisticLock({Bc}SearchCriteria criteria);
...
public class {Bc}LockQueryAdapter implements {Bc}LockQueryPort {
     ...
     */
    @Override
    public Optional<Bc> findByIdWithPessimisticLock({Bc}Id id) {
        return lockRepository.findByIdWithPessimisticLock(id.getValue())
            .map({bc}JpaEntityMapper::toDomain);
    }

    /**
     * Criteria로 Bc 목록 조회 (비관락)
     *
     ...
     */
    @Override
    public List<Bc> findByCriteriaWithPessimisticLock({Bc}SearchCriteria criteria) {
        List<{Bc}JpaEntity> entities = lockRepository.findByCriteriaWithPessimisticLock(criteria);
...
- [ ] 6개 메서드 구현 (비관락 2 + 낙관락 2 + For Update 2)
