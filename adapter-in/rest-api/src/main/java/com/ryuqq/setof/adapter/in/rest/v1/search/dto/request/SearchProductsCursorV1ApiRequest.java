 ...
 *
 * <p>레거시 SearchFilter (AbstractItemFilter 상속) 기반 변환. fetchSearchResults 전용 요청 DTO.
 *
 ...
 *
 * <p>기본값 처리는 Mapper에서 수행합니다. Request DTO에서는 기본값 설정 금지. orderType 미지정 시 Mapper에서 RECOMMEND로 자동
 * 설정됩니다.
 *
 * @param searchWord 검색 키워드 (MySQL ngram FULLTEXT 검색 대상 / null or blank 시 전체 상품 조회)
 * @param lastDomainId 커서 페이징 기준 ID (이전 페이지 마지막 productGroupId)
 * @param cursorValue 커서 값 (정렬 기준 값: 점수/날짜/가격/할인율 등)
 ...
 * @param orderType 정렬 기준 (미지정 시 RECOMMEND 자동 적용)
...
public record SearchProductsCursorV1ApiRequest(
        @Parameter(
                        description =
                                "검색 키워드 (MySQL ngram FULLTEXT 검색 / null 또는 빈 값 입력 시 전체 상품 조회)",
                        example = "나이키")
                String searchWord,
        @Parameter(description = "커서 기준 마지막 상품그룹 ID (다음 페이지 조회 시 사용)", example = "980")
                @Schema(description = "커서: 마지막 상품그룹 ID", nullable = true)
                Long lastDomainId,
        @Parameter(description = "커서 값 (정렬 기준 값: 점수/날짜/가격/할인율 등)", example = "87.5")
        ...
        @Parameter(description = "다중 브랜드 ID 필터 (최대 100개)")
                ...
                List<Long> brandIds,
        @Parameter(
                        description = "정렬 기준 (미지정 시 RECOMMEND 자동 적용)",
                        example = "RECOMMEND",
                        schema =
                                @Schema(
                                        allowableValues = {
