...
/**
 * SearchPaymentsCursorV1ApiRequest - 결제 목록 검색 요청 DTO (커서 페이징).
 *
 * <p>레거시 PaymentFilter 기반 변환.
 *
 ...
 * @param lastPaymentId 마지막으로 조회한 결제 ID (커서 페이징용)
 * @param size 조회할 아이템 수 (1~100)
 * @author ryu-qqq
 * @since 1.0.0
 * @see com.setof.connectly.module.payment.dto.filter.PaymentFilter
 */
@Schema(description = "결제 목록 검색 요청 (커서 페이징)")
public record SearchPaymentsCursorV1ApiRequest(
        ...
        @Parameter(description = "마지막으로 조회한 결제 ID (다음 페이지 조회 시 사용)", example = "1000")
                @Schema(description = "커서: 마지막 결제 ID", nullable = true)
                Long lastPaymentId,
