 ...
 *
 * <p>레거시 CreateQna(인터페이스) 기반 변환. PUT /api/v1/qna/{qnaId}/reply/{qnaAnswerId} 에 대응.
 *
 ...
 *
 * <p>qnaId, qnaAnswerId는 @PathVariable로 수신하므로 이 DTO에 포함하지 않습니다.
 *
 * <p>비즈니스 규칙: - CLOSED 상태의 답변은 수정할 수 없습니다 (Application 계층에서 검증). - 이미지 diff 처리: qnaImageId가 null이면
 * 신규 추가, 있으면 수정, 목록에 없는 기존 이미지는 소프트 삭제. - 추가 + 수정 이미지 합계가 3장을 초과하면 예외가 발생합니다.
 *
...
public record UpdateQnaReplyV1ApiRequest(
    ...
    /**
     * QnaImageRequest - Q&A 답변 첨부 이미지 수정 요청.
     *
     ...
     */
    @Schema(description = "Q&A 답변 첨부 이미지 수정 요청")
    public record QnaImageRequest(
            @Schema(description = "기존 이미지 ID (null이면 신규 추가)", example = "10", nullable = true)
                    Long qnaImageId,
            @Schema(description = "이미지 URL", example = "https://cdn.example.com/qna/image_new.jpg")
                    String imageUrl,
