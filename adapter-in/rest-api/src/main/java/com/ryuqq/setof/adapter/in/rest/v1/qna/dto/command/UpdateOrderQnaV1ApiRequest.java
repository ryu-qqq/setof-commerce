...
public record UpdateOrderQnaV1ApiRequest(
        ...
        @Schema(description = "Q&A 이미지 목록 (최대 3장)") @Size(max = 3)
                List<UpdateQnaImageV1ApiRequest> qnaImages)
    ...
    public record QnaContentsV1ApiRequest(
            ...
            @Schema(description = "내용", example = "배송 일정 수정 문의 내용입니다.") @Size(max = 500)
                    String content) {}

    @Schema(description = "Q&A 이미지 수정 정보")
    public record UpdateQnaImageV1ApiRequest(
            @JsonInclude(JsonInclude.Include.NON_NULL)
                    @Schema(description = "QnA 이미지 ID", example = "11")
                    Long qnaImageId,
            @JsonInclude(JsonInclude.Include.NON_NULL)
                    @Schema(description = "QnA ID", example = "1")
                    Long qnaId,
            @JsonInclude(JsonInclude.Include.NON_NULL)
                    @Schema(description = "QnA 답변 ID", example = "10")
                    Long qnaAnswerId,
