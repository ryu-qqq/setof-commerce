...
@Import({
    ShippingAddressV2Controller.class,
...
class ShippingAddressV2ControllerDocsTest extends RestDocsTestSupport {
    ...
    @MockitoBean private GetShippingAddressesUseCase getShippingAddressesUseCase;

    @MockitoBean private RegisterShippingAddressUseCase registerShippingAddressUseCase;

    @MockitoBean private GetShippingAddressUseCase getShippingAddressUseCase;

    @MockitoBean private UpdateShippingAddressUseCase updateShippingAddressUseCase;

    @MockitoBean private DeleteShippingAddressUseCase deleteShippingAddressUseCase;

    @MockitoBean private SetDefaultShippingAddressUseCase setDefaultShippingAddressUseCase;

    @MockitoBean private ShippingAddressV2ApiMapper shippingAddressV2ApiMapper;
                        ...
                        document(
                                ...
                                responseFields(
                                        ...
                                        fieldWithPath("data[].id")
                                                ...
                                                .description("배송지 ID"),
                                        fieldWithPath("data[].addressName")
                                                .type(JsonFieldType.STRING)
                                                .description("배송지 이름"),
                                        fieldWithPath("data[].receiverName")
                                                .type(JsonFieldType.STRING)
                                                .description("수령인 이름"),
                                        fieldWithPath("data[].receiverPhone")
                                                .type(JsonFieldType.STRING)
                                                .description("수령인 연락처"),
                                        fieldWithPath("data[].roadAddress")
                                                .type(JsonFieldType.STRING)
                                                .description("도로명 주소"),
                                        fieldWithPath("data[].jibunAddress")
                                                .type(JsonFieldType.STRING)
                                                .description("지번 주소")
                                                .optional(),
    ...
    void registerShippingAddress() throws Exception {
        ...
        // When & Then
        mockMvc.perform(
                        post(ApiV2Paths.ShippingAddresses.BASE)
                                .with(user(principal))
                                .with(csrf())
    ...
    void updateShippingAddress() throws Exception {
        ...
        when(shippingAddressV2ApiMapper.toUpdateCommand(any(UUID.class), any(Long.class), any()))
                .thenReturn(
                        UpdateShippingAddressCommand.of(
                                UUID.fromString(MEMBER_ID),
                                shippingAddressId,
                                "집",
                                "홍길동",
        ...
        mockMvc.perform(
                        put(
                                        ApiV2Paths.ShippingAddresses.BASE + "/{shippingAddressId}",
                                        shippingAddressId)
                                .with(user(principal))
                                .with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                ...
                .andDo(
                        document(
                                ...
                                pathParameters(
                                        parameterWithName("shippingAddressId")
                                                .description("배송지 ID")),
                                requestFields(
                                        fieldWithPath("addressName")
                                                .type(JsonFieldType.STRING)
    ...
    @Test
    @DisplayName(
            "PATCH /api/v2/members/me/shipping-addresses/{shippingAddressId}/delete - 배송지 삭제"
                    + " API 문서")
    void deleteShippingAddress() throws Exception {
        ...
        when(shippingAddressV2ApiMapper.toDeleteCommand(any(UUID.class), any(Long.class)))
                .thenReturn(
                        DeleteShippingAddressCommand.of(
                                UUID.fromString(MEMBER_ID), shippingAddressId));
        doNothing()
                .when(deleteShippingAddressUseCase)
                .execute(any(DeleteShippingAddressCommand.class));

        // When & Then
        mockMvc.perform(
                        patch(
                                        ApiV2Paths.ShippingAddresses.BASE
                                                + "/{shippingAddressId}"
                                                + ApiV2Paths.ShippingAddresses.DELETE_PATH,
                                        shippingAddressId)
                                .with(user(principal))
                                .with(csrf()))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName(
            "PATCH /api/v2/members/me/shipping-addresses/{shippingAddressId}/default - 기본 배송지"
                    + " 설정 API 문서")
    void setDefaultShippingAddress() throws Exception {
        ...
        when(shippingAddressV2ApiMapper.toSetDefaultCommand(any(UUID.class), any(Long.class)))
                .thenReturn(
                        SetDefaultShippingAddressCommand.of(
                                UUID.fromString(MEMBER_ID), shippingAddressId));
        ...
        mockMvc.perform(
                        patch(
                                        ApiV2Paths.ShippingAddresses.BASE
                                                + "/{shippingAddressId}"
                                                + ApiV2Paths.ShippingAddresses.DEFAULT_PATH,
                                        shippingAddressId)
                                .with(user(principal))
                                .with(csrf()))
                .andExpect(status().isOk())
