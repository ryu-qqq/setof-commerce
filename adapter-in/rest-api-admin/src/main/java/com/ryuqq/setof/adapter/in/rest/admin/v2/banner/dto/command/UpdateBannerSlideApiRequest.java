 ...
 *
 * <p>API-DTO-004: slideId는 null이면 신규 생성, 값이 있으면 기존 슬라이드 수정 (Diff 패턴).
 *
 * @param slideId 슬라이드 ID (null이면 신규 생성)
 * @param title 슬라이드 제목
 ...
 * @since 1.1.0
 */
@Schema(description = "배너 슬라이드 수정 요청")
public record UpdateBannerSlideApiRequest(
        @Schema(
                        description = "슬라이드 ID (null이면 신규 생성, 값이 있으면 기존 수정)",
                        example = "1",
                        nullable = true)
                Long slideId,
        @Schema(
