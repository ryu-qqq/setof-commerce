...
@JsonSubTypes({
    @JsonSubTypes.Type(value = ShipOrderV1ApiRequest.class, name = "shipOrder"),
    @JsonSubTypes.Type(value = ClaimOrderV1ApiRequest.class, name = "claimOrder"),
    @JsonSubTypes.Type(
            value = ClaimRejectedAndShipmentOrderV1ApiRequest.class,
...
public sealed interface UpdateOrderV1ApiRequest
        permits ShipOrderV1ApiRequest,
                ClaimOrderV1ApiRequest,
                ClaimRejectedAndShipmentOrderV1ApiRequest,
                NormalOrderV1ApiRequest {
...
record ShipOrderV1ApiRequest(
        ...
        @Schema(
                ...
                String orderStatus,
        @Schema(description = "우회 여부", example = "false") Boolean byPass,
        @Schema(description = "배송 정보") @jakarta.validation.Valid
                ShipmentInfoV1ApiRequest shipmentInfo)
        implements UpdateOrderV1ApiRequest {
...
record ClaimOrderV1ApiRequest(
        ...
        @Schema(description = "우회 여부", example = "false") Boolean byPass,
        @Schema(description = "변경 사유", example = "고객 요청")
                @Length(max = 200, message = "변경 사유는 200자를 넘을 수 없습니다.")
                String changeReason,
        @Schema(description = "변경 상세 사유", example = "단순 변심")
                @Length(max = 500, message = "변경 상세 사유는 500자를 넘을 수 없습니다.")
