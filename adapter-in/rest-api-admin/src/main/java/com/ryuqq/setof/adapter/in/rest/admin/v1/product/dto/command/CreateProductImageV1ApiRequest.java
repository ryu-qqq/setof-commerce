...
public record CreateProductImageV1ApiRequest(
        @Schema(description = "이미지 타입", example = "MAIN",
                requiredMode = Schema.RequiredMode.REQUIRED) @NotNull(
                        message = "이미지 타입은 필수입니다.") String productImageType,
        @Schema(description = "이미지 URL", example = "https://example.com/image.jpg",
                requiredMode = Schema.RequiredMode.REQUIRED) @NotBlank(
                        message = "이미지 URL은 필수입니다.") @Length(max = 255,
                                message = "이미지 URL은 255자를 넘을 수 없습니다.") String imageUrl,
        @Schema(description = "원본 URL", example = "https://example.com/origin.jpg") @Length(
                max = 255, message = "원본 URL은 255자를 넘을 수 없습니다.") String originUrl) {
}
