...
public record OrderAdminV2ApiResponse(
        ...
        @Schema(description = "주문 상품 목록") List<OrderItemAdminV2ApiResponse> items,
     ...
     * @return API 응답
     */
    public static OrderAdminV2ApiResponse from(OrderResponse response) {
        List<OrderItemAdminV2ApiResponse> itemResponses =
                response.items() != null
                        ? response.items().stream().map(OrderItemAdminV2ApiResponse::from).toList()
                        : List.of();

        return new OrderAdminV2ApiResponse(
                response.orderId(),
