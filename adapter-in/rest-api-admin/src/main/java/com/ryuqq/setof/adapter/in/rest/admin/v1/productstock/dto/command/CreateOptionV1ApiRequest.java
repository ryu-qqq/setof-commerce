package com.ryuqq.setof.adapter.in.rest.admin.v1.productstock.dto.command;
...
public record CreateOptionV1ApiRequest(
        @Schema(description = "상품 ID", example = "123") @JsonInclude(JsonInclude.Include.NON_NULL)
                Long productId,
        @Schema(description = "재고 수량", example = "100", requiredMode = Schema.RequiredMode.REQUIRED)
                @Max(value = 9999, message = "재고 수량은 9999를 넘을 수 없습니다.")
                @Min(value = 0, message = "재고 수량은 0 이상이어야 합니다.")
                Integer quantity,
        @Schema(description = "추가 가격", example = "5000") BigDecimal additionalPrice,
        @Schema(description = "옵션 상세 목록", requiredMode = Schema.RequiredMode.REQUIRED)
                @Valid
                @Size(max = 2, message = "옵션 상세는 최대 2개까지 가능합니다.")
                @NotNull(message = "옵션 상세 목록은 필수입니다.")
                List<CreateOptionDetailV1ApiRequest> options) {
