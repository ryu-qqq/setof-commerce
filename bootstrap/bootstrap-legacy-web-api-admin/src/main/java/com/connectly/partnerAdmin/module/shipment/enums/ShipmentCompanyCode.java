...
public enum ShipmentCompanyCode implements EnumType {
    ...
    public static ShipmentCompanyCode of(String code) {
        return Arrays.stream(ShipmentCompanyCode.values())
                .filter(shipmentCompanyCode -> shipmentCompanyCode.getName().equals(code))
                .findFirst()
                .orElse(ofCode(code));
    }

    public static ShipmentCompanyCode ofCode(String code) {
        return Arrays.stream(ShipmentCompanyCode.values())
                .filter(shipmentCompanyCode -> shipmentCompanyCode.getCode().equals(code))
                .findFirst()
                .orElse(ShipmentCompanyCode.REFER_DETAIL);
    }
