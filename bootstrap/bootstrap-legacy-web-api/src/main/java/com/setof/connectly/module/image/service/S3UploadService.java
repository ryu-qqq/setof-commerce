...
public class S3UploadService implements ImageUploadService{
    ...
    private static final String DEFAULT_IMAGE_URL = "https://cdn.set-of.com/logo/setof-logo.png";
    ...
    private CompletableFuture<String> uploadToS3(String objectKey, byte[] imageBytes, String contentType) {
    ...
    public CompletableFuture<String> uploadImage(ImagePath imagePath, String originalImageUrl) {
        ...
        if(originalImageUrl.contains(assetUrl)) return CompletableFuture.completedFuture(originalImageUrl);
        if (originalImageUrl.contains(tempUrl)) return moveImageFromTempToTargetBucket(originalImageUrl, imagePath, fileName);
    ...
    @Async
    public CompletableFuture<String> moveImageFromTempToTargetBucket(String originalImageUrl, ImagePath imagePath, String fileName) {
    ...
    private String generateObjectKey(ImagePath imagePath, String tempKey, String fileName) {
        ...
        String baseKey = imagePath.getPath() + todayDate + SLASH + (cleanFileName.isEmpty() ? UUID.randomUUID().toString() : cleanFileName);
        return extension.isEmpty() ? baseKey : baseKey + DOT + extension;
    }

    @Async
    public CompletableFuture<Void> copyImage(String sourceBucketName, String destinationBucketName, String sourceObjectKey, String destinationObjectKey) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                log.error("Error copying image from bucket {} to bucket {}, image name {}: {}", sourceBucketName, destinationBucketName, sourceObjectKey, e.getMessage());
    ...
    public CompletableFuture<Void> deleteImageFromBucket(String bucketName, String objectKey) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                log.error("Error deleting image from bucket {}: {}", bucketName, e.getMessage());
