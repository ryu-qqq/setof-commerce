...
public class DateUtils {
    ...
    public static String Lunar2Solar(String yyyymmdd) {
        ...
        cc.set(
                ...
                Integer.parseInt(date.substring(0, 4)) + LUNAR_YEAR_OFFSET);
        ...
        return String.format(
                "%04d%02d%02d",
                solar.getYear(), solar.getMonth().getValue(), solar.getDayOfMonth());
    }
    ...
    public static Set<String> holidayArray(LocalDateTime localDateTime) {
        ...
        // 음력 휴일 추가
        String prev_seol =
                LocalDate.parse(Lunar2Solar(yyyy + "0101"), formatter)
                        .minusDays(1)
                        .toString()
                        .replace("-", "");
    ...
    private static void addReplacementHolidays(
            Set<String> holidays, String year, DateTimeFormatter formatter) {
        ...
        // 설날 대체공휴일 검사
        if (LocalDate.parse(Lunar2Solar(year + "0101"), formatter).getDayOfWeek().getValue()
                == LD_SUNDAY) { // 일
        ...
        if (LocalDate.parse(Lunar2Solar(year + "0102"), formatter).getDayOfWeek().getValue()
                == LD_SUNDAY) { // 일
            holidays.add(Lunar2Solar(year + "0103"));
        }

        // 추석 대체공휴일 검사
        if (LocalDate.parse(Lunar2Solar(year + "0814"), formatter).getDayOfWeek().getValue()
                == LD_SUNDAY) {
            holidays.add(Lunar2Solar(year + "0817"));
        }
        if (LocalDate.parse(Lunar2Solar(year + "0815"), formatter).getDayOfWeek().getValue()
                == LD_SUNDAY) {
            holidays.add(Lunar2Solar(year + "0817"));
        }
        if (LocalDate.parse(Lunar2Solar(year + "0816"), formatter).getDayOfWeek().getValue()
                == LD_SUNDAY) {
            holidays.add(Lunar2Solar(year + "0817"));
        }
    }
    ...
    public static boolean isCancelImmediately(LocalDateTime updateTime, LocalDateTime nowTime) {
        LocalDateTime isImmediatelyCancelDay = getCancelDay(updateTime);
        return isImmediatelyCancelDay.isBefore(nowTime);
    }
}
